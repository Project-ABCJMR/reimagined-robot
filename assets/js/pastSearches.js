...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function loadPastSearches() {
    ...
    pastSearchesContent.innerHTML = pastSearches
      .map(
        (search) => `<button class="button past-search-btn">${search}</button>`
      )
      .join("");
  }
  ...
  function handlePastSearchClick(event) {
    ...
    if (target.classList.contains("past-search-btn")) {
      ...
      console.log("clicked search, target is year");
      if (isNaN(searchDate) === false) {
        cardConElem.style.display = "none";
        getEvent(searchDate);
        // Get the search date from the button and perform the search
      } else {
        cardConElem.style.display = "block";
